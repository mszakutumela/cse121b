...
// condition 1
    if (addNumber1 == answerOne) {
        ...
        ans1Element.innerHTML = `<strong>${wrongAnswer}</strong>`;
    }
// condition 2
    if (addNumber2 == answerTwo) {
        ans2Element.innerHTML = `<strong>${answerTwo}, correct answer</strong>`;
    } else {
...
/* async displayMilestone Function */

const displayMilestones = (temples) => {
    temples.forEach(temple => {      
    let article = document.createElement("article");                   
    let templeName = document.createElement("h3");
    let announced =
    let groundBreak =
    let dedicated =
    let     "imageUrl": "https://content.churchofjesuschrist.org/templesldsorg/bc/Temples/photo-galleries/durban-south-africa/400x250/4-5fcf64b0979ac41f7197d049ea8c4ffad88cd1b4.jpg"
    =
    templeName.textContent = temple.templeName;
